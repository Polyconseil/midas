...
def test_devices_metadata(client):
    ...
    response = client.options(
        reverse("agency-0.4:device-list"),
...
def test_device_list_basic(client, django_assert_num_queries):
    ...
    # test auth
    response = client.get(reverse("agency-0.4:device-list"))
    ...
    # test auth
    response = client.get(reverse("agency-0.4:device-detail", args=[device.id]))
    assert response.status_code == 401
    ...
    with django_assert_num_queries(n):
        response = client.get(
            reverse("agency-0.4:device-detail", args=[device.id]),
            **auth_header(SCOPE_AGENCY_API, provider_id=provider.id),
        )
    assert response.status_code == 200
    ...
    response = client.get(
        reverse("agency-0.4:device-detail", args=[other_device.id]),
        **auth_header(SCOPE_AGENCY_API, provider_id=provider.id),
    )
...
def test_device_register(client):
    ...
    response = client.post(
        reverse("agency-0.4:device-list"), data=data, content_type="application/json"
    )
    assert response.status_code == 401
...
def test_device_event(client):
    ...
    response = client.post(
        reverse("agency-0.4:device-event", args=[device_id]),
        data=data,
        content_type="application/json",
    )
    assert response.status_code == 401
...
@pytest.mark.django_db
@override_settings(
    ENABLE_TELEMETRY_FUNCTION="tests.apis.agency_api.v0_4.test_vehicles.return_false"
)
