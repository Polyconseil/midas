...
from mds.authent.public_api import *
...
class AppCreationView(GenericAPIView):
    ...
    class CreationRequestSerializer(serializers.Serializer):
        ...
        scopes = serializers.CharField(required=False, help_text="Scope of the application separated by a comma.", default=["provider"])
    ...
    class CreationResponseSerializer(serializers.Serializer):
        ...
        client_secret = serializers.CharField(help_text="Newly created Oauth app Client Secret")

    class RevocationRequestSerializer(serializers.Serializer):
        app_owner = serializers.UUIDField(
            help_text="The owner for which the application will be revoked."
        )
        delete = serializers.BooleanField(required=False, help_text="Indicate if the application should be removed or just revoked", default = False)
...
class LongLivedTokenView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = get_long_lived_token(application, serializer.validated_data["token_duration"])
...
urls = ([
    path(
        "long_lived_token/", LongLivedTokenView.as_view(), name="long_lived_token"
    ),
    path(
        "create_application/", AppCreationView.as_view(), name="create_app"
    ),
    path(
        "revoke_application/", AppCreationView.as_view(), name="revoke_app"
    ),
    path(
        "delete_application/", AppCreationView.as_view(), name="delete_app"
    ),
],"mds_prv_auth_api")